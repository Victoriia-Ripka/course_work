...
void cut_paste(input info, scene *a) //функція виводу реплік, яка приймає структуру даних, що ввів користувач і посилання на поточну сцену
{
    ...
    (*(a+10)).set(7, "\0", "10.00 a.m.. "+info.name+" came to Territorial Defense for asking \nto join the honorable TerDefense "
    "of Fayna Ukrayna under the command of his\nlong-time school friend, a major Serhiy Prytula.");
    ...
    (*(a+13)).set(8, "Serhiy Prytula","We have already no places, all posts have long been\n occupied by others. For 1 automaton - 300 willings.");
...
" Pitun was not ready for this, so came out\nto negotiate with Powderbot. He had known that a hetman, who came to him, \nis a real miser.");
    ...
    (*(a+64)).set(16, "\0", "Realizing the deplorable situation, Pitun began to flee from the positions,\nsacrificing his army for his own life. " + info.name + " started the chase.");
    ...
    (*(a+69)).set(8, "Serhiy Prytula", "Do not invent, as from yours " + info.numeric +" hryvnias subtract 1500,\nyou will not become much poorer. But that will"
    " go for an army, we will buy \n20 nuclear toilets of type “land-air” to close 4 positions. We’ll strike a \npreventive shot. Here I took the map, I'll show you.");

    (*(a+70)).set(19, "\0", "Under " + info.name + "'s command, a diversionary reconnaissance group\nwas set up to explore areas in the woods near his swamp. He chose that task \nfor himself,"
    " suspecting Pitun's true purposes. He guessed that that one \nwill come for " + info.noun + " personally. Armed, the soldiers \ntook up positions.");
    ...
    (*(a+82)).set(22, "\0", info.name + " protected his swamp and his lair, and didn’t give up\n" + info.noun + ". In gratitude to the soldiers"
    " for their cooperation, he gave one\nto each of them. “These affairs deserve such an award” - he considers.");
    (*(a+83)).set(22, "\0", "He sold the obtained " + info.body + " and earned a lot of money on it.\nGod took away, God thanked. 15000 of dollars were now heating in his wallet\n"
    "and a souvenir - Pitun's canned head, which always bubbled under the nose \nstrange random phrases like:\n" + pitun_speak(10+rand()%10));
    (*(a+84)).set(22, "\0", info.name + " received a medal “For Courage“ from the President of Fayna\n Ukrayna, put an end to universal evil,"
