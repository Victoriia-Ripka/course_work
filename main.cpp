...
int main()
{
    ...
    if(choice_f()){// В залежності від вибору починати з різних сцен
        for(int i=6; i<10; i++){ //ітераційний цикл що виводить в консоль сторінки історії
        show_scene (script[i]); //функція виводу ілюстрації, роздільної лінії та репліки в консоль
        answer();
    }
    } 
    ...
    if(stoi(info.numeric) < 1500){// Сюжетна розвилка
        for(int  i=20;i<68;i++){
        show_scene (script[i]); //функція виводу ілюстрації, роздільної лінії та репліки в консоль
        answer();
    }
