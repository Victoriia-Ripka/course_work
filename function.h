...
{
    cout << "\033[2J\033[1;1H"; //Переводить курсор в лівий верхній кут, створює новий слайд
    printscene(a.landscape);    //Візуалізація написананого
    if (a.name != NO_NAME)         //Перевірка, чи є спікер в сцені
    {
        line();
...
enum
{
    NPREF = 2,    //кількість елементів у контейнері префіксу 
    NHASH = 1000, // 4093, size of state hash table array 
...
string pitun_speak(int nwords) //функція, що генерує n випадкових слів типу денацифікація, біолабораторії та інші "скрєпні" речі
{
    ...
    srand(time(NULL));                 // сід для рандому
